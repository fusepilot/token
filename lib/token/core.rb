...
module Token
  ...
  class Render < AbstractController::Base
    ...
    def render_token token
      ...
      locals.merge! Token.options[token[:type].to_sym]
      p locals
      unless locals[:model].nil?
        model_name = locals[:model].constantize
        model = model_name.find(locals[:value])
        locals[locals[:model].downcase.to_sym] = model
      end
      p locals
      
      #merge in config parameters
      #locals[:image] = Image.find(1)
      return render :partial => locals[:partial], :locals => locals
    end
  end
